...
class MsgDevice(object):
    ...
    def _recv_reply(self, block=True):
        ...
        if msgs[0] == 'OK':
            ...
            import cPickle
            raise cPickle.loads(msgs[1])

    def _send_cmd(self, msgs, check_reply=True):
        blocked = False
        try:
            self.pair.send_multipart(msgs, zmq.NOBLOCK)
    ...
    def set_repub_interval(self, interval):
        if 0.1<=interval<=10.0:
            ...
            raise MsgDeviceException("repub interval must be >=0.1s or <=10.0s, got "+str(interval))
...
class MsgThread(threading.Thread):
    ...
    def _proc_pair_sock(self):
        try:
            while 1:
                msgs = self.pair.recv_multipart(zmq.NOBLOCK)
    ...
    def _cmd_add_sub_url(self, msgs):
        self.sub.setsockopt(zmq.SUBSCRIBE, msgs[1])
        self._reply_ok()
    ...
    def _send_pending_pubs(self):
        if len(self.pending_pubs)>0:
            try:
                for kv in self.pending_pubs.items():
    ...
    def _check_new_pub(self):
        if not self.has_new_pub: return

        pubs = self.dev.get_pub_dict()
        self.has_new_pub = False
        for k in pubs:
            v = self._pack(pubs[k])
            if k not in self.pub_dict \
                or v != self.pub_dict[k]:
                # sent pubs only if not same as cache version
                self.pub_dict[k] = v
                self.pending_pubs[k] = v
                self.wait_repubs.pop(k,0)
    ...
    def _check_repub(self):
        ...
        if now <= self.next_repub_time:
            ...
            if(self.poll_time_ms<1): self.poll_time_ms = 1
        else:
    ...
    def run(self):
        try:
            ...
            if hasattr(self,'pair'):
                import cPickle
                self.pair.send_multipart(('ERROR',cPickle.dumps(e,-1))) 
        finally:
            self._thr_final()